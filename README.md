...
- **`v11_final.ipynb`** — the main application. Loads every model and launches the GUI. It also
...
Open `v11_final.ipynb` in Colab, pick a GPU runtime, and run the cells in order (Sections 0 to 14):
...
- **Role CNN** → upload `best_cnn_classifier.pth` to use it, or train it from the "Train CNN" tab
  (needs the `volley_detection-5` dataset in `/content/`).
